...
interface AddInLikedPostButtonType {
  coffeeId: string;
}

const AddInLikedPostButton = ({ coffeeId }: AddInLikedPostButtonType) => {
  const dispatch = useAppDispatch();
  const { authData } = useSelector(getAuthData);
  const likedPosts: Array<string> = authData?.likedPosts
    ? authData?.likedPosts
    : [];

  const userData = {
    id: authData?.id,
    likedPosts: [...likedPosts, coffeeId],
  };
  const AddInCart = () => {
...
export default AddInLikedPostButton;
